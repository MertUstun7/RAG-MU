...
class VectorDb:
    ...
    def similarity_search(
        self, query: str, initial_k: int = DEFAULT_INITIAL_K, final_k: int = DEFAULT_FINAL_K
    ) -> list[Document]:
    ...
    def add_documents(self, documents: list[dict]) -> dict:
        ...
        chunker = RecursiveCharacterTextSplitter(chunk_size=1024, chunk_overlap=100)
        ...
        for document in documents:
            ...
            for page in document["pages"]:
                ...
                try:
                    ...
                    logger.warning(
                        "Semantic chunking failed for %s p.%s, skipping:%s",
                        document_name,
                        page_number,
                        e,
                    )
        ...
        try:
            ...
            if existing and existing["documents"]:
                for content, metadata in zip(
                    existing["documents"], existing["metadatas"], strict=False
                ):
                    if content:
