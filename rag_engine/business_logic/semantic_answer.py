...
class SemanticAnswer:
    ...
    def _get_rag_result(self, query):
        ...
        for semantic_result in semantic_results:
            ...
            if semantic_result["has_image"]:
                images.append(
                    str(semantic_result["doc_name"]) + "_p" + str(semantic_result["page_num"]) + "_"
                )
        ...
        if images:
            ...
            print(len(images))
            if len(images_res) > 1:
                image_info = "Additionally, the images below illustrate this: " + str(
                    [item["context"] for item in images_res]
                )
                chunks.append(image_info)
            elif len(images_res) == 1:
                image_info = "Additionally, the image below illustrates this: " + str(
                    images_res[0]["context"]
                )
                chunks.append(image_info)
    ...
    def rag_llm_results(self, query):
        if not self.collection_name:
            ...
            return {"llm_response": response["message"]["content"], "images": ""}
        ...
        return {"llm_response": response["message"]["content"], "images": image_list, "docs": docs}
