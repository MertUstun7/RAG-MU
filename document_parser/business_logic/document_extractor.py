...
class PageBasedExtractor:
    ...
    def _save_image(self, picture_item, page_no, base_name, img_index):
        ...
        if hasattr(picture_item, "image") and picture_item.image is not None:
            if (
                hasattr(picture_item.image, "pil_image")
                and picture_item.image.pil_image is not None
            ):
        ...
        uri = None
        if (
            hasattr(picture_item, "image")
            and picture_item.image
            and hasattr(picture_item.image, "uri")
        ):
